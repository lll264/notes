...
**MySQL 规定除了 TEXT、BLOBs 这种大对象类型之外，一行记录（不包括隐藏列和记录头信息）占用的字节长度加起来不能超过 65535 个字节**。
...
只有叶子节点（最底层的节点）才存放了数据，**非叶子节点（其他上层节）仅用来存放索引**
...
但是备份就会花费很多的时间，关键是备份期间，业务只能读数据，而不能更新数据，这样**会造成业务停滞**。
...
加锁情况为：索引1上的临键锁（-∞，1】，索引5上的临建锁（1，5】，索引10上的间隙锁（5，10)。
