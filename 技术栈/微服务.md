...
数据在进行网络传输的时候，需要进行序列化，序列化协议有很多种，比如xml, json，protobuf等。gRPC默认使用`protocol buffers`，这是google开源的一套成熟的结构数据序列化机制。在学习gRPC之前，需要先了解`protocol buffers`
...
option go_package = "hello_client/proto"; //项目中import导入生成的代码名称
...
新建一个hello_server项目，编写上一个步骤中的proto文件
...
import (
	...
	"net"
	"project2/grpctest/hello_server/pb"
)
...
func (s *server) SayHello(ctx context.Context, request *pb.HelloRequest) (*pb.HelloResponse, error) {
	return &pb.HelloResponse{Reply: "Hello " + request.GetName()}, nil
}
...
import (
	...
	"project2/grpctest/hello_client/proto"
	"time"
)
