...
Go 自带的正则表达式不支持一些语法，比如说我这里想要用的表达式：
^(?=.*[A-Za-z])(?=.*\d)(?=.*[$@$!%*#?&])[A-Za-z\d$@$!%*#?&]{8,}$类似于 ?=. 这种就不支持。所以我们换用另外一个开源的正则表达式匹配库：**github.com/dlclark/regexp2。**

```go
//校验邮箱
emailExp := regexp2.MustCompile(emailRegexPattern, regexp2.None)
ok, _ := emailExp.MatchString(req.Email) //与邮箱字符串进行校验
if !ok {
